...
summarised100 <- summarised100[match(c("IS", "WO-Replacement-$S_1$", "Bootstrap-Auxiliary", "WO-Replacement-$S_{2, 1}$", "Bootstrap"), summarised100$method),]
summarised1000 <- summarised1000[match(c("IS", "WO-Replacement-$S_1$", "Bootstrap-Auxiliary", "WO-Replacement-$S_{2, 1}$", "Bootstrap"), summarised1000$method),]

colnames(summarised100) <- colnames(summarised1000) <- c("Method", "MSE", "RE")
print(xtable(summarised100, display = c("s", "s", "e", "e"), label = "table:particleBernoulli_100", caption = "Simulation results for a binomial probability, with $\\sampleSize = 100$"), include.rownames=FALSE, math.style.exponents = TRUE, sanitize.text.function = identity)
print(xtable(summarised1000, display = c("s", "s", "e", "e"), label = "table:particleBernoulli_1000", caption = "Simulation results for a binomial probability, with $\\sampleSize = 1000$"), include.rownames=FALSE, math.style.exponents = TRUE, sanitize.text.function = identity)
